...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private class ProgressTask extends AsyncTask<HRCardRecForm, Integer, ArrayList<HRCardRecForm>> {

        private static final String LOG_ACTIVITY_TAG = "ProgressTask";
        //private final Context MAIN_ACTIVITY_CONTEXT = MainActivity;
        ...
        @Override
        protected ArrayList<HRCardRecForm> doInBackground(HRCardRecForm... params) {
            //my stuff is here

            //Log.w(LOG_ACTIVITY_TAG, " IN doInBackground ");
            HRCardRecService hrCardRecService = new HRCardRecService();
            HRCardRecForm hrcardrecform_thread = params[0];
            ArrayList<HRCardRecForm> qHRCardRecs;
            hrCardRecService.queryHRRec(hrcardrecform_thread);
            qHRCardRecs = (ArrayList<HRCardRecForm>) hrCardRecService.getHRCardRecForms();
            //Log.w(LOG_ACTIVITY_TAG, "qHRCardRecs length: " + qHRCardRecs.size());
            return qHRCardRecs;
        ...
        @Override
        protected void onPostExecute(ArrayList<HRCardRecForm> result) {
            progressDialog.dismiss();
            super.onPostExecute(result);
            Log.w(LOG_ACTIVITY_TAG, " POST EXECUTE");
        }
    }
}

//use msghandler to handle msg from sub-thread to main thread GUI
